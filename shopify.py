...
def get_page(url, page, collection_handle=None):
    ...
    full_url += '/products.json' #?sort_by=best-selling
...
def print_as_csv(url, collections):
    ...
    # writer.writerow(['URL','Code', 'Collection', 'Category',
    #                  'Name', 'Variant Name',
    #                  'Price', 'In Stock', 'URL', 'Image URL', 
    #                 #  'Body'
    #                 ])
    for product in extract_products(url, collections):
        writer.writerow([url, product['code'], product['collection'],
...
def main():
    ...
    parser.add_argument('--list-collections', '-l', dest='list_collections', action='store_true', help='List collections in the site')
    parser.add_argument('--batch', '-b', default='', dest='batch', help='Get all products in many threads')
    parser.add_argument('--collections', '-c', dest='collections', default='', help='Get products only from the given collections (comma separated)')
    parser.add_argument('--output-format', '-of', dest='output_format', default='csv', choices=['csv', 'json'], help='Get output in either csv(default) or json')
    parser.add_argument('--url', '-u' , default='',help='URL of the shopify site')
    ...
    if args.list_collections:
        ...
        if args.batch:
            with open(args.batch) as f:
                ...
                results = pool.map(batch_process, urls)
